...
export default class Sync {
  constructor({
    ...
    //const oauthToken = 'BQBAsGX3-Wj6VPOsO9zSSH3V26E6hfvDJQYF-sp__YSjRMPzOKIWNVxPNL2ir3dn7B7qn_OHsCjNGn5GvGTlnbqE0zYZsTga4jBuarJKx5CnEaMcDeXjik87iC_c2cdccALWCp0UswC6M1TDhjF2Dg';

    this.state = Observe({
      api: {
        currentlyPlaying: 'https://api.spotify.com/v1/me/player',
        nextTrack: 'https://api.spotify.com/v1/me/player/next',
  ...
  /**
   * @method skipSong - skip to next song
   */
  async skipSong() {
    try {
      this.state.api.headers = {
      ...
      const { data } = await post(this.state.api.nextTrack, { headers: this.state.api.headers })
      if (!data) {
...
function getAuthId() {
  return Math.random().toString(36).slice(5, 11)
}


export async function auth() {
  console.log(`Project root: ${PROJECT_ROOT}`);
  window.location.href = `${PROJECT_ROOT}/login?auth_id=${getAuthId()}`
}
